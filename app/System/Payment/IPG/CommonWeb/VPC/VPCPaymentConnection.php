...
namespace System\Payment\IPG\CommonWeb\VPC;

/**
 * Class VPCPaymentConnection
 * @package Services\Payment\IPG\CommonWeb\VPC
 * @version 2.0
 */
class VPCPaymentConnection
{
    ...
    protected $sslVerifyPeer = 0; //turn on/off cert validation 0 = don't verify peer, 1 = do verify
    protected $sslVerifyHost = 0; // 0 = don't verify hostname, 1 = check for existence of hostame, 2 = verify
    ...
    public function addDigitalOrderField($field, $value)
    {
        ...
        if (strlen($value) == 0) {
            return false;
        }      // Exit the function if no $value data is provided
        if (strlen($field) == 0) {
            return false;
        }      // Exit the function if no $value data is provided
    ...
    public function sendMOTODigitalOrder($vpcURL, $proxyHostAndPort = "", $proxyUserPwd = "")
    {
        ...
        if ($this->isSSL) {
            /* This sets the path to certificate file */
    ...
    public function decryptDR($digitalReceipt)
    {
        ...
        if (!$this->socketCreated) {
            return false;
        }        // Exit function if an the socket connection hasn't been created
        if ($this->errorExists) {
            return false;
        }           // Exit function if an error exists
